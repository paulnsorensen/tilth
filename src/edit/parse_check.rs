...
//!
//! After a `tilth_write` section applies, parse the pre- and post-edit content
//! with the language's tree-sitter grammar and surface any *new* `ERROR` /
//! `MISSING` nodes the edit introduced. Pre-existing errors stay silent.
//! Adapted from lack435/tilth `edit_parse_check.rs` (end-state at 81c3a72) —
//! the multiset-diff design ports directly onto our whole-file-tag apply path;
//! their per-line hash-anchor edit model does not carry over.
//!
...
mod tests {
    ...
    fn edit_inside_preexisting_unterminated_region_is_silent() {
        ...
        // so the multiset key stays stable across an edit beneath it.
    ...
    fn dockerfile_returns_none() {
        // Dockerfile is detected but has no tree-sitter grammar.
