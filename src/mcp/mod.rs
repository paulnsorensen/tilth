...
fn tool_read(
    ...
    let parsed: Vec<(PathBuf, crate::mcp::path_suffix::PathSuffix)> = raw_paths
        .iter()
        .map(|s| crate::mcp::path_suffix::parse_path_with_suffix(s))
        .collect();
    let paths: Vec<PathBuf> = parsed.iter().map(|(p, _)| p.clone()).collect();
    let suffixes: Vec<&crate::mcp::path_suffix::PathSuffix> =
        parsed.iter().map(|(_, s)| s).collect();

    let now = std::time::SystemTime::now();
    let has_any_suffix = suffixes
        .iter()
        .any(|s| !matches!(s, crate::mcp::path_suffix::PathSuffix::None));
        ...
        if has_any_suffix || since.is_some() || force_signature || force_full || mode_str == "auto"
        {
            ...
            for (path, suffix) in &parsed {
                ...
                let signature = force_signature
                    || (!force_full
                        && mode_str == "auto"
                        && matches!(suffix, crate::mcp::path_suffix::PathSuffix::None)
                        && should_auto_signature(path));
                let body =
                    if force_full && matches!(suffix, crate::mcp::path_suffix::PathSuffix::None) {
    ...
    let suffix = suffixes
        ...
        .next()
        .cloned()
        .unwrap_or(crate::mcp::path_suffix::PathSuffix::None);
    ...
    if !matches!(suffix, crate::mcp::path_suffix::PathSuffix::None) {
        session.record_read(&path);
    ...
    if force_signature || (!force_full && mode_str == "auto" && should_auto_signature(&path)) {
        return Ok(apply_budget(
            read_single_with_suffix(
                &path,
                &crate::mcp::path_suffix::PathSuffix::None,
                true,
                edit_mode,
                cache,
            ),
...
fn read_single_with_suffix(
    path: &Path,
    suffix: &crate::mcp::path_suffix::PathSuffix,
    signature: bool,
    edit_mode: bool,
    cache: &OutlineCache,
) -> String {
