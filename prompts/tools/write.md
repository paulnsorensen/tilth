...
Example with `diff: true` (verify what landed, no follow-up read): `tilth_write(files: [{path: "src/foo.rs", mode: "hash", edits: [{start: "42:a1b", content: "let x = 2;\n"}]}], diff: true)` — response includes a `── diff ──\n--- before\n+++ after\n- <old line>\n+ <new line>` block per file.
...
If the diff reveals another needed edit, DO NOT call tilth_write again immediately — collect every visible follow-up, then issue ONE grouped follow-up call (after a read/search verification step if needed).
