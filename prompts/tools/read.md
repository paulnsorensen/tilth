...
A leading JSON header may report view/truncation metadata; reuse its timestamp for unchanged stubs. `next_view` means content is hidden; read the needed `path#symbol` or `path#n-m` section, not `mode=full`.