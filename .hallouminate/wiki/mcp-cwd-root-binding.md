...
How tilth resolves relative paths to the *right* checkout, why it uses a **required per-call `cwd` param** instead of the MCP `roots` capability, and the git-worktree gotcha that motivated it. Full cited research at `.cheese/research/mcp-cwd-root-binding/` and `.cheese/research/mcp-roots-trust-signal/` (local, gitignored). History: issue #78 / PR #79 (`root`, required-for-relatives) → PR #113 `cwd-param-posture` (2026-07-04) → **PR #144 (hook removed; no host exempt, current law)**.

## Current law (PR #113 + #144, spec `cwd-param-posture`)

- The param is named **`cwd`** (renamed from `root`) and is **schema-required on all seven path-taking tools** — `tilth_read`, `tilth_search`, `tilth_write`, `tilth_list`, `tilth_deps`, `tilth_grok`, `tilth_diff` (diff gained it in the `cwd-param-posture` work; **every git-backed source now diffs against `cwd`** via an explicit `checkout: &Path` threaded through `src/diff/` — `run_git_diff`, `git log`, `git show` blob reads, working-tree fs reads, `repo_root`/scope normalization, conflict reads, and blast caller search all run against the requested repo, never the server's frozen dir. Earlier the git-based sources diffed the frozen process dir — the D2 limitation, cured by the two-repo regression in `tests/mcp_v2/test_diff_cwd_binding.py`. The file-path params `patch`/`a`/`b` still anchor relative spellings under `cwd` like every other tool).
...
- **No host is exempt** (PR #144): the Claude Code hook plugin (`plugin/claude/`) and the `TILTH_MCP_CWD_HOOK_INJECTED` env var that flipped the `cwd` schema description were both deleted, so explicit per-call `cwd` is the only mechanism on every host including Claude Code. The prompt and the per-tool schema now carry one unconditional text. Operators who ran the old `tilth install claude-code` keep an orphaned hook on disk; `install` warns about it but does not remove it.
...
tilth is a long-lived stdio MCP server, spawned once per session. It cannot *pull* the caller's current directory: the OS cwd of the tilth process is frozen at spawn; the MCP `tools/call` frame carries no cwd field; the directory the agent "is in" lives in another process or only in the agent's intent. So the live cwd must be *pushed* by the caller — an explicit argument on each call.
...
- #113–#144 era: a Claude Code `PreToolUse` hook plugin (`plugin/claude/`, `hooks/inject-cwd.js`, matcher `mcp__tilth__.*`) injected the live session cwd via `hookSpecificOutput.updatedInput`, deferring whenever the model set `cwd` itself; `TILTH_MCP_CWD_HOOK_INJECTED` (written per-host by `tilth install`) flipped the prompt and schema text to "do NOT set it" on Claude Code only. Removed by PR #144: the injected value was the *session root*, which does not follow a subagent into a git worktree, so an omitted `cwd` was silently misrouted instead of hitting the teaching refusal (#143) — the exact silent-fallback failure this page argues against. Two model-facing texts also had to be kept in lockstep with a `.replace` swap that could no-op on markdown drift.
...
- Issue <https://github.com/paulnsorensen/tilth/issues/78> · PR <https://github.com/paulnsorensen/tilth/pull/79> · PR <https://github.com/paulnsorensen/tilth/pull/113> · **PR <https://github.com/paulnsorensen/tilth/pull/144>** (current)
- Issue <https://github.com/paulnsorensen/tilth/issues/143> — the worktree misroute the hook caused. Closed by construction for new installs in #144; an existing install keeps the orphaned hook until `tilth install claude-code` is re-run, which warns but does not remove it.
